...
import { ArrowLeft, Share2, ExternalLink, Shield, Settings, Users, Sun, Moon } from 'lucide-react'
...
export default function SellerProfilePage() {
  ...
  const fetchSellerProfile = useCallback(async () => {
    try {
      ...
      if (response.success && response.data) {
        ...
        // Set the page title (store name only)
        document.title = `${response.data.storeSettings?.storeName || 'Store'} - Seller Profile | Ishq Gems`
      } else {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `${profile?.storeSettings?.storeName || 'Store'} - Seller Profile`,
  ...
  // Extract store colors for gradient background
    ...
    <div 
      ...
      style={{
        background: `
          ...
          linear-gradient(135deg, ${primaryColor}08 0%, ${secondaryColor}05 50%, ${primaryColor}03 100%)
        `
      }}
    >
      ...
      <div className="container mx-auto px-4 py-8 relative z-10">
          ...
          <div className="flex items-center gap-2">
            ...
            {isOwnProfile && (
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 text-white rounded-lg hover:opacity-90 transition-all duration-200 shadow-lg backdrop-blur-sm"
                style={{
            ...
            <button
              onClick={toggleTheme}
              className="p-2 border border-border rounded-lg text-muted-foreground hover:bg-secondary hover:text-foreground transition-all duration-200 group"
        ...
        {isOwnProfile && (
          <div 
            className="mb-6 backdrop-blur-sm border rounded-xl p-4 shadow-lg"
            style={{
          ...
          >
            <div className="flex items-center gap-2" style={{ color: primaryColor }}>
              ...
              <span className="font-medium">Your Store Profile</span>
            </div>
            <p className="text-foreground/80 text-sm mt-1">
              This is how buyers and other users see your store profile on the platform. You can edit your store information from your dashboard.
            </p>
          </div>
        )}

        {isAdmin && !isOwnProfile && (
          <div className="mb-6 bg-amber-50/80 dark:bg-amber-900/20 backdrop-blur-sm border border-amber-200/50 dark:border-amber-800/50 rounded-xl p-4 shadow-lg">
            <div className="flex items-center gap-2 text-amber-700 dark:text-amber-400">
              <Shield className="w-5 h-5" />
              <span className="font-medium">Admin View</span>
            </div>
            <p className="text-amber-600 dark:text-amber-300 text-sm mt-1">
              You&apos;re viewing this store&apos;s profile with administrative privileges. You can manage this seller from the admin dashboard.
            </p>
          </div>
        )}

        {isSeller && !isOwnProfile && !isAdmin && (
          <div className="mb-6 bg-green-50/80 dark:bg-green-900/20 backdrop-blur-sm border border-green-200/50 dark:border-green-800/50 rounded-xl p-4 shadow-lg">
            <div className="flex items-center gap-2 text-green-700 dark:text-green-400">
              <Users className="w-5 h-5" />
              <span className="font-medium">Seller Network</span>
            </div>
            <p className="text-green-600 dark:text-green-300 text-sm mt-1">
              You&apos;re viewing a fellow seller&apos;s store profile. Connect and build relationships within the Ishq Gems seller community.
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-4 sm:top-6 lg:top-8 z-10">
              <SellerProfileCard
                profile={profile}
