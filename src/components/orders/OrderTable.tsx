        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 mb-3 sm:mb-4">
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-medium text-sm sm:text-base">Filters & Search</h3>
            </div>
            {hasActiveFilters && (
              <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="text-xs sm:text-sm text-primary hover:text-primary/80 transition-colors px-2 py-1 rounded-lg hover:bg-secondary/50"
          >
          ...
          <div className="flex-1">
            <div className="relative">
              <input
                ...
                className="w-full pl-9 sm:pl-10 pr-4 py-2 sm:py-2.5 rounded-lg border border-border bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-sm"
              />
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                <input
                  ...
                  className="w-full pl-8 sm:pl-10 pr-3 py-2 rounded-lg border border-border bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-sm"
                  placeholder="From date"
                />
                <Calendar className="absolute left-2 sm:left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 sm:w-4 sm:h-4 text-muted-foreground" />
              </div>
              <div className="relative flex-1">
                <input
                  type="date"
          ...
          <div className="flex flex-wrap gap-2 mt-3 sm:mt-4">
            ...
            {['pending', 'paid', 'processing', 'shipped', 'delivered', 'cancelled', 'refunded', 'returned'].map((status) => (
              <button
                ...
                className={`px-2 sm:px-3 py-1 rounded-full text-xs font-medium capitalize transition-colors ${
        ...
        <table className="w-full min-w-[800px]">
          <thead>
            <tr className="border-b border-border/30">
              <th className="px-2 sm:px-3 lg:px-4 py-2 sm:py-3 text-left text-xs sm:text-sm font-medium text-muted-foreground">Order</th>
              <th className="px-2 sm:px-3 lg:px-4 py-2 sm:py-3 text-left text-xs sm:text-sm font-medium text-muted-foreground hidden sm:table-cell">Date</th>
              <th className="px-2 sm:px-3 lg:px-4 py-2 sm:py-3 text-left text-xs sm:text-sm font-medium text-muted-foreground hidden md:table-cell">Buyer</th>
              <th className="px-2 sm:px-3 lg:px-4 py-2 sm:py-3 text-left text-xs sm:text-sm font-medium text-muted-foreground">Items</th>
              <th className="px-2 sm:px-3 lg:px-4 py-2 sm:py-3 text-left text-xs sm:text-sm font-medium text-muted-foreground">Total</th>
              <th className="px-2 sm:px-3 lg:px-4 py-2 sm:py-3 text-left text-xs sm:text-sm font-medium text-muted-foreground">Status</th>
              <th className="px-2 sm:px-3 lg:px-4 py-2 sm:py-3 text-left text-xs sm:text-sm font-medium text-muted-foreground">Actions</th>
            </tr>
          </thead>
          <tbody>
            {filteredOrders.length === 0 ? (
                  ...
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div>
                      ...
                      {hasActiveFilters && (
                        <button
                          onClick={clearAllFilters}
                          className="mt-2 text-xs sm:text-sm text-primary hover:text-primary/80 transition-colors"
                        >
                          Clear all filters
                        </button>
                      )}
              ...
              <React.Fragment key={order._id}>
                <tr className="border-b border-border/30 hover:bg-secondary/30">
                  ...
                  <td className="px-2 sm:px-3 lg:px-4 py-2 sm:py-3 text-xs sm:text-sm hidden sm:table-cell">
                  ...
                  <td className="px-2 sm:px-3 lg:px-4 py-2 sm:py-3 hidden md:table-cell">
                    <div className="flex items-center gap-1.5 sm:gap-2">
                      ...
                      />
                      <span className="text-xs sm:text-sm truncate">{order.buyerDetails.name}</span>
                    </div>
                  </td>
                  <td className="px-2 sm:px-3 lg:px-4 py-2 sm:py-3 text-xs sm:text-sm">{order.totalItems}</td>
                  <td className="px-2 sm:px-3 lg:px-4 py-2 sm:py-3">
                ...
                {expandedOrders.has(order._id) && (
                  <tr>
                    <td colSpan={7} className="bg-secondary/30 px-2 sm:px-3 lg:px-4 py-3 sm:py-4">
                      <div className="space-y-3 sm:space-y-4">
                        {order.subOrders.map((subOrder) => (
                          <div key={subOrder._id} className="bg-card rounded-lg p-3 sm:p-4">
                            {/* Seller Info */}
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 mb-3 sm:mb-4">
                              <div className="flex items-center gap-2 sm:gap-3">
                                <S3Image
                                ...
                                <div className="min-w-0">
                                  <h4 className="font-medium text-sm sm:text-base truncate">{subOrder.sellerDetails.name}</h4>
                                  <p className="text-xs sm:text-sm text-muted-foreground">
                              ...
                              <div className="flex items-center gap-2">
                                <span className={`px-2 sm:px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(subOrder.status)}`}>
                                  {subOrder.status}
                                </span>
                                <button
                                  className="p-1 hover:bg-secondary rounded transition-colors"
                              ...
                              {subOrder.items.map((item) => (
                                <div key={item._id} className="flex items-start gap-2 sm:gap-3">
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <h5 className="font-medium text-xs sm:text-sm truncate">{item.gemDetails.name}</h5>
